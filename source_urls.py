...
@lru_cache(maxsize=1)
def _dead_urls() -> frozenset[str]:
    """Load the denylist of URLs that returned 404/410 at validation time.

    Generated by scripts/validate_source_urls.py. Anything on this list is
    suppressed from get_url() so the UI never surfaces a broken link.
    """
    if not _DEAD_URL_FILE.exists():
        return frozenset()
    try:
        urls = set()
        for line in _DEAD_URL_FILE.read_text(encoding="utf-8").splitlines():
            line = line.strip()
...
def get_url(filename: str) -> str | None:
    ...
    or if the URL is on the dead-link denylist."""
    if not filename:
        return None
